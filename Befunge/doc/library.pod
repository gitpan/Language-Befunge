# $Id: library.pod,v 1.1 2002/04/13 11:07:05 jquelin Exp $
...
B</!\> Warning: in order for the interpreter to find your cool module,
you are to put it somewhere in C<@INC>.
...
the I<cartesian Lahey space topology> (in order to fetch values, move
the IP, or whatever), each function is called with two arguments:

=over 4

=item o

the current Instruction Pointer (see L<Language::Befunge::IP> for
more documentation about what you can do with it).

=item o

the LaheySpace where the Instruction Pointer moves (see
L<Language::Befunge::LaheySpace> for more documentation about the
relevant methods).
...
instruction! Let's code it just now:
    ...
    sub S {
        my $ip = shift; # no need to fetch the Lahey space.
...
serie of characters onto the stack, but in fact we're storing the
B<ordinal> values of the chars. This way, they will be displayed the
right way when used with C<,>.


=head2 Be kind to your mates

Nothing else is needed for your cool extension to work. But since you
want to provide your module to everyone, I recommend you to:
...
overloaded, and what is the new semantics of the binded instructions.
...
=head2 There's no way to call another instruction within an
instruction

You can't call a builtin instruction while you're processing an
overloaded instruction. This means you can't call C<a> to store the
value C<10> onto the stack: you'll have to do it yourself (check the
Befunge.pm module in order to see how the instructions are processed,
copy & paste).


=head2 One instruction occurs in one tick

Remember that what you're implementing will be processed in one
...
should push C<ordinal> values of characters onto the stack, instead of
